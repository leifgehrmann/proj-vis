...
import getObjectFitContainSize from '../letterbox';
...
function updateMarkerTransformations() {
  ...
  if (props.lonValues.length === 0) {
    containerElement.textContent = '';
    return;
  }

  // Type definitions need to be declared because for some reason `watch` does
  // not care about array order.
  const { canvas, toCanvas, toCoords } = createValidCoordinatesCanvas(
    props.step as number,
    props.lonValues as number[],
    props.latValues as number[],
    props.colorValues as Uint8ClampedArray[],
  );
...
onMounted(() => {
  ...
  containerElement.addEventListener('click', (event) => {
    ...
    markerCoordinateModel.value = canvasToCoord.value(clientToCanvas.value(
